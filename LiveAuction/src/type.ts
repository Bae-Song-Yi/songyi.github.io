...
export interface AucStateResType {
  ...
  claim_bid_hst_seq: number;
}

// 작품리스트 조회
export interface AuctionWorkResponse {
  w_make_date: string;
  ...
  w_size: string;
  successful_bid_price: number;
  successful_bid: boolean; // 낙찰여부. false가 낙찰
  is_fair_warning: boolean;
  work_seq: number;
  isWish: boolean;
}
...
export interface tSeqResType {
  t_seq: number; // 작품 종류 키
  t_name: string; // 작품 종류 이름
}
...
export interface currentBidClaimType {
  ...
  bid_stat_cd: string;
  bid_reg_dat: string;
  bid_chg_reason_memo: string;
  bid_chg_date: string;
  bid_noti_memo: string;
}

export interface MyBidsType {}
