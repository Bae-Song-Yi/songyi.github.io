...
export function UnrealWebRTCPlayer(
  videoID,
  alias,
  sid,
  ipAddress,
  ...
  this.Play = function Play() {
    ...
    if (state == -1 && nowTime - latestStopTime > 1000) {
      ...
      connOK = false;
      ToggleOverlay(false);
      remoteVideo.setAttribute('style', 'background: black center no-repeat;');
  ...
  function onCreateOfferSuccess(desc) {
    ...
    if (audioCodec != '') desc.sdp = setCodec(desc.sdp, 'audio', audioCodec, audioRate);
    if (videoCodec != '') desc.sdp = setCodec(desc.sdp, 'video', videoCodec, 90000);
  ...
  function gotRemoteStream(e) {
    ...
    if (videoCodec == '') remoteVideo.setAttribute('style', 'background-color:black');
  ...
  //DoSignaling 함수내에서  Terminate; 다음에 setTimeout(function () { that.Play(); }, 3000);  삽입-3곳
    ...
    ws.onmessage = function (evt) {
      ...
      if (state == 0) {
        ...
        if (strArr.length == 1) {
          ...
          if (videoCodec != '' && audioCodec != '') offerOptions = { offerToReceiveAudio: 1, offerToReceiveVideo: 1 };
          else if (videoCodec != '') offerOptions = { offerToReceiveAudio: 0, offerToReceiveVideo: 1 };
          else if (audioCodec != '') offerOptions = { offerToReceiveAudio: 1, offerToReceiveVideo: 0 };
