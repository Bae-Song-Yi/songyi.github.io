...
└─ supabase/
   ...
   └─ functions/
      ...
      └─ faq-chat/                  # 신청 폼 챗봇 자유질문 → Gemini 답변 (Edge Function)
```
...
- **우측 하단 도움말 챗봇**: FAQ 버튼은 즉답(AI 불필요), 자유 입력은 Gemini가 서비스 안내 기반으로 답변
...
### 5. 신청 폼 챗봇 (선택)
1. `faq-chat` 함수 배포 — **Verify JWT 끄기** (`GEMINI_API_KEY`는 4번에서 이미 등록됨)
2. 우측 하단 💬 → FAQ 버튼은 배포 없이도 즉답, 자유 입력은 함수 배포 후 AI 답변
   - 챗봇이 답할 서비스 정보(참가비·매칭·개인정보 등)는 `faq-chat/index.ts`의 시스템 프롬프트에서 수정
