$(document).ready(function(){
    function cafeteriaContainer(){
        const cafeLogin = $('.cafe-login article');
        const reservation = $('.cafe-reservation article');
        const luchorderBtn = $('.lunch-order .apply-btn');
        const bottomMenu = $('.cafe-reservation .bottom-menu');
        const windowHT = $(window).height();
        
        cafeLogin.css('height', windowHT);
        reservation.css({'min-height': windowHT});
        $('.apply-menu-detail').css({'min-height': windowHT, 'padding-bottom': $('.apply-wrap').outerHeight() +  bottomMenu.outerHeight() + 38});
        $('.cafe-reservation .apply-wrap').css('padding-bottom', bottomMenu.outerHeight())
    }
    cafeteriaContainer();
    ...
    function findDay(){
        ...
        if (calendarTab.find('a.one-day').hasClass('on')) { // 일일
            /* $('.lunch-order span').text(`${firstdayMonth}.${firstDay}(${firstWeek})`); */
            $('.lunch-order b').hide();
            //$('.lunch-order span').append($('<i/>').text(`${lastdayMonth}.${lastDay}(${lastWeek})`))

        } else if (calendarTab.find('a.one-week').hasClass('on')) { // 일주일
            //$('.lunch-order span').text(`${firstdayMonth}.${firstDay}(${firstWeek}) ~ ${lastdayMonth}.${lastDay}(${lastWeek})`);
            //dayActive.css('background', '#F76E33');
        } 
    }
